...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        #data = (x-sample_mean)/np.sqrt(sample_var + eps)
        #out = gamma*data + beta
        """
        sample_mean = np.mean(x, axis=0)
        ...
        data = (x-running_mean)/np.sqrt(running_var)
        out = data*gamma + beta
...
def batchnorm_backward_alt(dout, cache):
    ...
    dxhat = dout * gamma
    dx = 1.0/N * 1/np.sqrt(var + eps) * (N*dxhat - np.sum(dxhat, axis=0) - xhat*np.sum(dxhat*xhat, axis=0))
      ...
      for f in range(F):
        for hprime in range(Hprime):
          for wprime in range(Wprime):
            ...
            w1 = wprime * stride
            w2 = wprime * stride + WW
            window = xpad[:, h1:h2, w1:w2]
            out[n, f, hprime, wprime] = np.sum(window * w[f,:,:,:]) + b[f]
